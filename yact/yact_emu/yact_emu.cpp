...
UINT32 peldrexecaccess(int prm_0, int prm_1, int prm_2) {
	...
	case 0:
		if (prm_0 < 4096) {
			...
			VirtualQuery((void*)prm_0, &mbi, sizeof(mbi));
			if (mbi.State & 0x10000) {
				descptx4tmp[prm_0] = prm_1; return 0;
				//memcpy((void*)(prm_0+(&descptx4tmp)), &prm_1, (1 << (prm_2 >> 4))); return 0;
				//*((UINT8*)(prm_0+(&descptx4tmp))) = prm_1; return 0;
			}
		}
		/*printf("%08X\n", prm_0);
		printf("%08X\n", CPU_EIP);*/
		memdirectaccess[prm_0] = prm_1;
		//memcpy((void*)prm_0, &prm_1, (1<<(prm_2>>4)));
		//*((UINT8*)(prm_0)) = prm_1;
		return 0;
		break;
	case 1:
		if (prm_0 < 4096) { 
			...
			VirtualQuery((void*)prm_0, &mbi, sizeof(mbi));
			if (mbi.State & 0x10000) {
				//return (UINT32)((void*)(prm_0 + (&descptx4tmp)));
				return descptx4tmp[prm_0];
				//memcpy(&buf4readmem,((void*)(prm_0 + (&descptx4tmp))), (1 << (prm_2 >> 4)));
				//return *((UINT8*)(prm_0 + (&descptx4tmp)));
			}
		}
		/*printf("%08X\n", prm_0);
		printf("%08X\n", CPU_EIP);*/
		//return (UINT32)((void*)(prm_0));
		return memdirectaccess[prm_0];
		//memcpy(&buf4readmem, ((void*)(prm_0)), (1 << (prm_2 >> 4)));
			//return *((UINT8*)(prm_0));
		break;
	case 3:
...
EMU_EXPORT DWORD EmuExecute(DWORD Addr, int NParams, ...)
{
		...
		FS_BASE = (DWORD)PeLdrGetCurrentTeb();
		FPU_CTRLWORD = GetFPUCW();
	...
	*(DWORD*)CPU_ESP = CbReturnToHost();
	...
	while(true){
		...
		exec_1step();//else { do{ if (CbIsReturnToHost(CPU_EIP)) { break; } exec_1step(); } while (CPU_REMCLOCK > 0); CPU_REMCLOCK = 0x7FFFFFFF; }
